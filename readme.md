...
The application provides a `/healthz` endpoint that returns a `200 OK` status. This endpoint can be used by load balancers, container orchestrators (e.g., Kubernetes liveness/readiness probes), or other monitoring tools to check the liveness and readiness of the service.

To check the health of the service, simply send an HTTP GET request to `http://<your-service-ip>:<port>/healthz`.

Example:
```bash
curl http://localhost:9990/healthz
```
